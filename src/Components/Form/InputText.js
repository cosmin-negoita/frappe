...
export default function Input(props) {
    ...
    const classes = clsx(componentBaseClass, "input--text", hasError, size, className);

    return (
        <div className="form-field">
            {props.label && <label className="text--label">{props.label}</label>}
            <input size={props.size} type="text" name={props.name} className={classes} placeholder={props.placeholder} value={props.value} onChange={props.onChange} disabled={props.disabled} />
